...
#include <Open3DViewer\MouseSelection.h>
#include <Open3DViewer\SoMouseWheelEvent.h>
#include <Open3DViewer\Type.h>
#include <Open3DViewer\MenuManager.h>
...
void NavigationStyle::setViewer(View3DInventor* view)
{
	this->viewer = view;
}
...
void NavigationStyle::spin(const SbVec2f & pointerpos)
{
	...
	auto vpr = viewer->getViewportRegion();
	auto origin = vpr.getViewportOriginPixels();
...
void NavigationStyle::openPopupMenu(const SbVec2s& position)
{
	...
	Gui::MenuItem* view = new Gui::MenuItem;
	...
	Gui::MenuManager::getInstance()->setupContextMenu(view, contextMenu);

	if (MainViewer::mainViewer->actionsList)
		contextMenu.addActions(*MainViewer::mainViewer->actionsList);
	if (MainViewer::mainViewer->menus)
		for (auto eachmenu : *MainViewer::mainViewer->menus)
			contextMenu.addMenu(eachmenu);
	...
	if (used && subMenuGroup.actions().indexOf(used) >= 0 && used->isChecked()) {
		...
		//while (widget && !widget->inherits("View3DInventor"))

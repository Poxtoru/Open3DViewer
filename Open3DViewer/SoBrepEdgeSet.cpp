...
struct SoBrepEdgeSet::SelContext : SoFCSelectionContext {
...
void SoBrepEdgeSet::GLRender(SoGLRenderAction *action)
{
	...
	SelContextPtr ctx2;
	SelContextPtr ctx = SoSelectionRoot::getRenderContext<SelContext>(this, selContext, ctx2);
	...
	if (action->getTypeId() == SoHighlightElementAction::getClassTypeId()) {
		...
		case SoSelectionElementAction::Remove: {
			...
			if (selaction->getType() == SoSelectionElementAction::Append) {
				ctx = SoSelectionRoot::getActionContext(action, this, selContext);
				selCounter.checkAction(selaction, ctx);
				ctx->selectionColor = selaction->getColor();
				if (ctx->isSelectAll())
					ctx->selectionIndex.clear();
				...
				ctx = SoSelectionRoot::getActionContext(action, this, selContext, false);

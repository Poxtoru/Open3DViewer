...
SoFullPath * SoFCUnifiedSelection::currenthighlight = NULL;
...
SoSelectionRoot::Stack SoSelectionRoot::SelStack;
std::unordered_map<SoAction*, SoSelectionRoot::Stack> SoSelectionRoot::ActionStacks;
SoSelectionRoot::ColorStack SoSelectionRoot::SelColorStack;
SoSelectionRoot::ColorStack SoSelectionRoot::HlColorStack;
SoSelectionRoot* SoSelectionRoot::ShapeColorNode;

SO_NODE_SOURCE(SoSelectionRoot)

SoSelectionRoot::SoSelectionRoot(bool trackCacheMode)
	:SoFCSeparator(trackCacheMode)
{
	SO_NODE_CONSTRUCTOR(SoSelectionRoot);
...
SoSelectionRoot::~SoSelectionRoot()
{
}

void SoSelectionRoot::initClass(void)
{
	SO_NODE_INIT_CLASS(SoSelectionRoot, SoFCSeparator, "SelectionRoot");
...
void SoSelectionRoot::finish()
{
	so_bbox_cleanup();
	atexit_cleanup();
}

SoNode *SoSelectionRoot::getCurrentRoot(bool front, SoNode *def) {
	if (SelStack.size())
		return front ? SelStack.front() : SelStack.back();
	return def;
}

SoFCSelectionContextBasePtr SoSelectionRoot::getNodeContext(
	Stack &stack, SoNode *node, SoFCSelectionContextBasePtr def)
{
	if (stack.empty())
		return def;

	SoSelectionRoot *front = stack.front();

	// NOTE: _node is not necessary of type SoSelectionRoot, but it is safe
	// here since we only use it as searching key, although it is probably not
	// a best practice.
	stack.front() = static_cast<SoSelectionRoot*>(node);
...
SoSelectionRoot::getNodeContext2(Stack &stack, SoNode *node, SoFCSelectionContextBase::MergeFunc *merge)
{
	...
	stack.back() = static_cast<SoSelectionRoot*>(node);
...
std::pair<bool, SoFCSelectionContextBasePtr*> SoSelectionRoot::findActionContext(
	SoAction *action, SoNode *_node, bool create, bool erase)
{
	...
	auto node = static_cast<SoSelectionRoot*>(_node);
...
bool SoSelectionRoot::renderBBox(SoGLRenderAction *action, SoNode *node, SbColor color)
{
...
void SoSelectionRoot::renderPrivate(SoGLRenderAction * action, bool inPath) {
...
bool SoSelectionRoot::_renderPrivate(SoGLRenderAction * action, bool inPath) {
	...
	if ((style == SoSelectionRoot::Box /*|| ViewParams::instance()->getShowSelectionBoundingBox()*/)
		&& ctx && !ctx->hideAll && (ctx->selAll || ctx->hlAll))
	{
		if (style == SoSelectionRoot::PassThrough) {
			style = SoSelectionRoot::Box;
		}
		else {
	...
	if (!ShapeColorNode && overrideColor &&
		...
		(style == SoSelectionRoot::Box || !ctx || (!ctx->selAll && !ctx->hideAll)))
	{
		ShapeColorNode = this;
		colorPushed = true;
		state->push();
...
void SoSelectionRoot::GLRenderBelowPath(SoGLRenderAction * action) {
	renderPrivate(action, false);
}

void SoSelectionRoot::GLRenderInPath(SoGLRenderAction * action) {
	if (action->getCurPathCode() == SoAction::BELOW_PATH)
		return GLRenderBelowPath(action);
	renderPrivate(action, true);
}

bool SoSelectionRoot::checkColorOverride(SoState *state) {
	if (ShapeColorNode) {
...
void SoSelectionRoot::checkSelection(bool &sel, SbColor &selColor, bool &hl, SbColor &hlColor) {
	sel = false;
	hl = false;
...
void SoSelectionRoot::resetContext() {
	contextMap.clear();
}

void SoSelectionRoot::moveActionStack(SoAction *from, SoAction *to, bool erase) {
...
#define BEGIN_ACTION \
    ...
    if(!stack.nodeSet.insert(this).second) \
    {\
        std::time_t t = std::time(0);\
        if(_CyclicLastReported < t) {\
            _CyclicLastReported = t+5;\
...
void SoSelectionRoot::pick(SoPickAction * action) {
	BEGIN_ACTION;
...
void SoSelectionRoot::rayPick(SoRayPickAction * action) {
	BEGIN_ACTION;
	if (doActionPrivate(stack, action))
...
void SoSelectionRoot::handleEvent(SoHandleEventAction * action) {
	BEGIN_ACTION;
	inherited::handleEvent(action);
	END_ACTION;
}

void SoSelectionRoot::search(SoSearchAction * action) {
	BEGIN_ACTION;
	inherited::search(action);
	END_ACTION;
}

void SoSelectionRoot::getPrimitiveCount(SoGetPrimitiveCountAction * action) {
	BEGIN_ACTION;
	inherited::getPrimitiveCount(action);
	END_ACTION;
}

void SoSelectionRoot::getBoundingBox(SoGetBoundingBoxAction * action)
{
	BEGIN_ACTION;
	if (doActionPrivate(stack, action))
...
void SoSelectionRoot::getMatrix(SoGetMatrixAction * action) {
	BEGIN_ACTION;
	if (doActionPrivate(stack, action))
...
void SoSelectionRoot::callback(SoCallbackAction *action) {
	BEGIN_ACTION;
	inherited::callback(action);
	END_ACTION;
}

void SoSelectionRoot::doAction(SoAction *action) {
	BEGIN_ACTION
		if (doActionPrivate(stack, action))
...
bool SoSelectionRoot::doActionPrivate(Stack &stack, SoAction *action) {
	...
	// SoSelectionRoot is found here. New function checkSelection() is exposed
	...
	if (action->isOfType(SoSelectionElementAction::getClassTypeId())) {
		...
		if (selAction->getType() == SoSelectionElementAction::None) {
			if (action->getWhatAppliedTo() == SoAction::NODE) {
				...
				// are the first SoSelectionRoot encountered (which means all
...
int SoSelectionRoot::SelContext::merge(int status, SoFCSelectionContextBasePtr &output,
	SoFCSelectionContextBasePtr input, SoSelectionRoot *)
{
...
SoFCUnifiedSelection::SoFCUnifiedSelection() /*: pcDocument(0)*/
{
	...
	preSelection = -1;
	useNewSelection = /*ViewParams::instance()->getUseNewSelection()*/true;
}
...
void SoFCUnifiedSelection::applySettings()
{
	...
	bool enablePre = /*hGrp->GetBool("EnablePreselection", true)*/true;
	bool enableSel =/* hGrp->GetBool("EnableSelection", true)*/true;
	if (!enablePre) {
...
SoFCUnifiedSelection::getPickedList(SoHandleEventAction* action, bool singlePick) const
{
	...
	for (int i = 0, count = points.getLength(); i < count; ++i) {
		...
		if (View3DInventor::inventorInstance && path && path->containsPath(action->getCurPath())) {
			vp = View3DInventor::inventorInstance->getViewProviderByPathFromHead(path);
		...
		info.vpd = static_cast<ViewProviderPartExt*>(vp);
...
void SoFCUnifiedSelection::doAction(SoAction *action)
{
	...
	if (action->getTypeId() == SoFCHighlightAction::getClassTypeId()) {
		...
		if (!setPreSelection && hilaction->SelChange.Type == SelectionChanges::RmvPreselect) {
			...
			//ViewProvider* vp = hilaction->SelChange.vp;
	...
	if (action->getTypeId() == SoFCSelectionAction::getClassTypeId()) {
				...
				|| selaction->SelChange.Type == SelectionChanges::RmvSelection))
		{
			...
			ViewProvider* vp = selaction->SelChange.vp;
...
bool SoFCUnifiedSelection::setHighlight(SoFullPath *path, const SoDetail *det,
	ViewProvider *vpd, const char *element, float x, float y, float z)
{
	...
	if (path && path->getLength())
	{
		...
		int ret = Selection().setPreselect(vpd, element, x, y, z);
...
bool SoFCUnifiedSelection::setSelection(const std::vector<PickedInfo> &infos, bool ctrlDown) {
	...
	if (ctrlDown) {
		if (Selection().isSelected(info.vpd, info.element.c_str(), 0))
			Selection().rmvSelection(info.vpd, info.element.c_str(), &sels);
		else {
			bool ok = Selection().addSelection(info.vpd,
	...
	const char *subSelected = Selection().getSelectedElement(
		vpd, subName.c_str());
	...
	bool ok = Selection().addSelection(info.vpd, subName.c_str(),
		pt[0], pt[1], pt[2], &sels);
	if (ok)
...
bool SoSelectionRoot::StackComp::operator()(const Stack &a, const Stack &b) const {
		...
		if (det)
			...
			if (!bbox) {
				for (int i = 0, count = path->getLength(); i < count; ++i) {
					if (!path->getNode(i)->isOfType(SoSelectionRoot::getClassTypeId()))
						continue;
					auto node = static_cast<SoSelectionRoot*>(path->getNode(i));
					if (node->selectionStyle.getValue() == SoSelectionRoot::Box) {
						bbox = true;
						break;
					}
				}
			...
			if (!bbox)
				...
				SoSelectionRoot::checkSelection(sel, selColor, hl, hlColor);
				if (sel || hl)
					SoSelectionRoot::renderBBox(action, this, hl ? hlColor : selColor);
				else
					inherited::GLRenderInPath(action);
			}
		}
...
void SoFCPathAnnotation::getBoundingBox(SoGetBoundingBoxAction * action)
{
	if (path) {
		...
		SoSelectionRoot::moveActionStack(action, &bboxAction, false);
		bboxAction.apply(path);
		SoSelectionRoot::moveActionStack(&bboxAction, action, true);

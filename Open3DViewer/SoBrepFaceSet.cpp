...
void SoBrepFaceSet::doAction(SoAction* action)
{
	if (action->getTypeId() == SoHighlightElementAction::getClassTypeId()) {
		...
		if (!hlaction->isHighlighted()) {
			SelContextPtr ctx = SoSelectionRoot::getActionContext(action, this, selContext, false);
			if (ctx) {
		...
		if (!detail) {
			SelContextPtr ctx = SoSelectionRoot::getActionContext(action, this, selContext);
		...
		case SoSelectionElementAction::All: {
			SelContextPtr ctx = SoSelectionRoot::getActionContext<SelContext>(action, this, selContext);
			...
			if (selaction->isSecondary()) {
				if (SoSelectionRoot::removeActionContext(action, this))
					touch();
			}
			else {
				SelContextPtr ctx = SoSelectionRoot::getActionContext(action, this, selContext, false);
		...
		case SoSelectionElementAction::Color:
			if (selaction->isSecondary()) {
				...
				auto ctx = SoSelectionRoot::getActionContext(action, this, selContext, false);
				if (colors.empty()) {
					if (ctx) {
						ctx->colors.clear();
						if (ctx->isSelectAll())
							SoSelectionRoot::removeActionContext(action, this);
						touch();
					}
					return;
				}
				...
				if (colors.begin()->first.empty() || colors.lower_bound(element) != colors.end()) {
					if (!ctx) {
						ctx = SoSelectionRoot::getActionContext<SelContext>(action, this);
						selCounter.checkAction(selaction, ctx);
						ctx->selectAll();
					}
		...
		case SoSelectionElementAction::Append: {
			...
			if (!detail || !detail->isOfType(SoFaceDetail::getClassTypeId())) {
				if (selaction->isSecondary()) {
					...
					// here.
					auto ctx = SoSelectionRoot::getActionContext<SelContext>(action, this);
					selCounter.checkAction(selaction, ctx);
					touch();
				}
				return;
			...
			if (selaction->getType() == SoSelectionElementAction::Append) {
				auto ctx = SoSelectionRoot::getActionContext(action, this, selContext);
				selCounter.checkAction(selaction, ctx);
				ctx->selectionColor = selaction->getColor();
				if (ctx->isSelectAll())
					ctx->selectionIndex.clear();
	...
	else if (action->getTypeId() == SoVRMLAction::getClassTypeId()) {
	...
	else if (action->getTypeId() == SoUpdateVBOAction::getClassTypeId()) {
...
void SoBrepFaceSet::GLRender(SoGLRenderAction *action)
{
	...
	SelContextPtr ctx2;
	SelContextPtr ctx = SoFCSelectionRoot::getRenderContext<SelContext>(this, selContext, ctx2);
...
void SoBrepFaceSet::GLRender(SoGLRenderAction *action)
{
	...
	SelContextPtr ctx = SoSelectionRoot::getRenderContext(this, selContext, ctx2);
	if (ctx2 && ctx2->selectionIndex.empty())
		return;
	...
	if (this->shouldGLRender(action)) {
		...
		if (hasVBO) {
			...
			SoGLVBOActivatedElement::get(state, hasVBO);
			//
...
bool SoBrepFaceSet::overrideMaterialBinding(SoGLRenderAction *action, SelContextPtr ctx, SelContextPtr ctx2) {
			...
			(ctx2 && ctx2->colors.size())))
	{
		...
		if (ctx && Selection().needPickedList()) {
			hasTransparency = true;
			if (trans0 < 0.5)
				trans0 = 0.5;
			trans_size = 1;
...
void SoBrepFaceSet::getBoundingBox(SoGetBoundingBoxAction * action) {
	...
	SelContextPtr ctx2 = SoSelectionRoot::getSecondaryActionContext<SelContext>(action, this);

...
#define NS GestureNavigationStyle
...
public:
    AwaitingMoveState(my_context ctx):my_base(ctx)
    {
        ...
        ns.mouseMoveThreshold = 10/*App::GetApplication().GetParameterGroupByPath
...
public:
    GestureState(my_context ctx):my_base(ctx)
    {
        ...
        ns.pan(ns.viewer->getSoRenderManager()->getCamera());//set up panningplane
        this->ratio = ns.viewer->getSoRenderManager()->getViewportRegion().getViewportAspectRatio();
        enableTilt = false/*!(App::GetApplication().GetParameterGroupByPath
...
TYPESYSTEM_SOURCE(GestureNavigationStyle, UserNavigationStyle)
...
{
    this->logging = false/*App::GetApplication().GetParameterGroupByPath
